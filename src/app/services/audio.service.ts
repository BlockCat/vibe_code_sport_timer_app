...
export class AudioService {
  ...
  playExerciseIntro(exerciseId: string, activeSeconds: number): void {
  ...
  playRandomHint(exerciseId: string): void {
    const hints = data.exercises[exerciseId as keyof typeof data.exercises]?.hints;
  ...
  private playAudio(src: string): void {
    ...
    audio.play().catch(error => console.error('Error playing audio:', error, src));
  }
}
